...
def rgb2xyz(rgb_image, device):
    ...
    res = torch.matmul(mt, temp_img.permute(1, 0, 2, 3).contiguous().view(3, -1)).view(3, rgb_image.size(0), rgb_image.size(2),
...
def ciede2000_diff(lab1, lab2, device):
    ...
    T = 1. - 0.17 * torch.cos(radians(aHP - 39)) + 0.24 * torch.cos(radians(2. * aHP)) + 0.32 * torch.cos(radians(3. * aHP + 6.)) - 0.2 * torch.cos(
        radians(4. * aHP - 63.))
    ...
    res_square = ((dLP / (sL * kL)) ** 2.) + ((dCP / (sC * kC)) ** 2.) * mask_0_no + ((dHP / (sH * kH)) ** 2.) * mask_0_no + rT * (
...
def deltaE(x, y):
    # compute mean (i.e., averaged on all Batches and pixels) CIE deltaE 2000
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # x = x.to(device) if x.device.type != device.type else x
    # y = y.to(device) if y.device.type != device.type else y

    return ciede2000_diff(rgb2lab_diff(expand_4d(x), device), rgb2lab_diff(expand_4d(y), device), device).mean()/10
